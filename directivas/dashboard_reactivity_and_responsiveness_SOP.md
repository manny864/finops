...
- Al usar `ResponsiveContainer` con `height="100%"`, el contenedor padre inmediato DEBE tener una altura explícitamente definida (por ejemplo, `h-64`, `h-72` o estar posicionado como absoluto con `inset-0` dentro de un contenedor relativo con altura fija). Si el padre inmediato es un flexbox sin altura explícita, Recharts fallará y el gráfico desaparecerá o mostrará advertencias en la consola del navegador.
- El valor del selector global de suscripciones es `"All"` (con la "A" mayúscula) cuando no hay ninguna suscripción específica seleccionada. Sin embargo, los endpoints de API y algunos dropdowns locales pueden utilizar `"all"` (en minúsculas). Se debe normalizar esta conversión (por ejemplo, si es `"All"`, enviar vacío o `"all"` a la API según corresponda).
