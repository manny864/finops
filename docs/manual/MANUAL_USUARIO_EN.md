...
| **Compute FinOps Cockpits (VMs, VMSS, App Services, Function Apps)** | Business/Enterprise | Granular optimization and governance modules: **Function Apps** (`/intelligence/computo/fapps`) with invocation telemetry, GB-seconds, log audits, and sampling/downgrade playbooks; **App Services** (`/intelligence/computo/waas`) with app density and packing; **VMSS** (`/intelligence/computo/vmss`) with autoscale and Spot instances; and **VMs** (`/intelligence/computo/vms`) with rightsizing and Hybrid Benefit. |
