...
| **Cockpits de Computação (VMs, VMSS, App Services, Function Apps)** | Business/Enterprise | Módulos de otimização e governança granular: **Function Apps** (`/intelligence/computo/fapps`) com telemetria de invocações, GB-s, auditoria de logs e playbooks de sampling/downgrade; **App Services** (`/intelligence/computo/waas`) com densidade de apps e empacotamento; **VMSS** (`/intelligence/computo/vmss`) com autoscale e instâncias Spot; e **VMs** (`/intelligence/computo/vms`) com rightsizing e Hybrid Benefit. |
