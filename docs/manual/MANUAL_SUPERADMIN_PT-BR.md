...
- **Cockpits FinOps de Computação e Eficiência Serverless:** inclusão de painéis resolutivos para **Function Apps** (`/intelligence/computo/fapps`) com telemetria de invocações, GB-s, auditoria de dependências (Storage/App Insights) e remediação com playbooks de sampling e downgrade; **App Services** (`/intelligence/computo/waas`) com densidade de apps e empacotamento; e **VMSS** (`/intelligence/computo/vmss`) com autoscale e instâncias Spot.
