...
export default async function AzureRedHatOpenShiftFinopsCmpPage() {
  ...
  const tAro = await getTranslations("AroFinopsCmp");
  return (
    <div className="content animate-in fade-in px-6 py-8">
      ...
      <div className="mb-6">
        <h2 className="text-xl font-bold text-[#1B2A41] dark:text-slate-100 flex items-center gap-2">
          <IconServer2 className="w-5 h-5 text-[#0054A6] dark:text-blue-400 shrink-0" />
          <span>{t("aroFinopsTitle")}</span>
          <InfoTooltip content={tAro("tooltip_page_header")} position="bottom" align="left" />
        </h2>
