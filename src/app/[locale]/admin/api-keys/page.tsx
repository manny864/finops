...
export default function PublicApiKeysPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!json.success) setError(json.error || "Error loading keys");
  ...
  const createKey = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        setError(json.error || "Error creating key");
      } else {
  ...
  const deleteKey = async (id: number) => {
    if (!confirm("Delete this key? Applications using it will stop working.")) return;
    if (!selectedTenant?.id) return;
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        setError(json.error || "Error deleting key");
      } else {
  ...
  const toggleKey = async (id: number, enabled: boolean) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        setError(json.error || "Error updating key");
      } else {
    ...
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Unlock className="w-6 h-6" /> API Pública
        </h1>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1 text-justify">
          Manage versioned REST API keys (`pak_xxx`) for public access to FinOps endpoints.
          Each key is assigned scopes and rate limits.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium mb-2">Scopes</label>
