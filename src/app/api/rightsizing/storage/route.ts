...
import { getStorageTieringRecommendations } from "@/modules/collectors/azure/storageTieringService";
...
export async function GET(request: NextRequest) {
    try {
        ...
        const data = await getWithStaleWhileRevalidate(
            `storage-tiering:v2:${tenantId}`,
            () => getStorageTieringRecommendations(tenantId),
            1800,
            600
        );
