...
export async function GET(request: NextRequest) {
  return runPrewarmSqlFinops(request);
}
...
async function runPrewarmSqlFinops(request: NextRequest) {
  try {
    ...
    for (const tenant of tenants) {
      ...
      try {
        const url = `${origin}/api/intelligence/databases/sql-metrics?tenantId=${encodeURIComponent(tenant.id)}&bust=1`;
    ...
    return NextResponse.json({
      status: "Azure SQL FinOps prewarm completed",
