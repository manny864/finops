...
export async function GET(request: NextRequest) {
  return runPrewarmPostgresFinops(request);
}
...
async function runPrewarmPostgresFinops(request: NextRequest) {
  try {
    ...
    for (const tenant of tenants) {
      ...
      try {
        const url = `${origin}/api/intelligence/databases/postgres-metrics?tenantId=${encodeURIComponent(tenant.id)}&bust=1`;
    ...
    return NextResponse.json({
      status: "PostgreSQL FinOps prewarm completed",
