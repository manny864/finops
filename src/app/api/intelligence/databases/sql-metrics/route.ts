    ...
    const instances = await Promise.all(
      billableResources.map(async (resource) => {
        ...
        try {
          ...
          const metricsUrl = `https://management.azure.com${resource.id}/providers/Microsoft.Insights/metrics?api-version=2018-01-01&metricnames=${metricNames.join(",")}&timespan=PT24H&interval=PT1H&aggregation=Average`;
