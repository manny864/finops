    ...
    if (monthlyCost > 0 && suggestions.length === beforeCount) {
      ...
      suggestions.push({
        title: isLikelyNonProd
          ...
          : "Revisar compromiso y rightsizing de PostgreSQL",
        instanceId: instance.id,
...
function generateMockPostgresHistory(seedOffset: number): MetricHistoryPoint[] {
  ...
  for (let i = 23; i >= 0; i--) {
    ...
    const dailyPattern = hours >= 9 && hours <= 18 ? 1.35 : 0.62;
    const sineWave = Math.sin((hours + seedOffset) * 0.27);
    const noise = 1 + Math.sin(i * 1.3) * 0.08;
    const loadFactor = Math.max(0.1, dailyPattern * (1 + sineWave * 0.2) * noise);

    points.push({
      timestamp,
      cpu_percent: round2(Math.min(99.9, Math.max(1.2, 32 * loadFactor + (seedOffset % 4)))),
      memory_percent: round2(Math.min(99.9, Math.max(10, 48 * loadFactor + 22 + (seedOffset % 10)))),
      active_connections: Math.round(Math.max(2, 40 * loadFactor + (seedOffset % 12))),
      connections_failed: Math.random() > 0.96 ? Math.round(Math.random() * 2) : 0,
      storage_percent: round2(Math.min(99.9, Math.max(5, 42 + i * 0.05 + Math.sin(hours) * 0.1))),
      io_consumption_percent: round2(Math.min(99.9, Math.max(0.5, 18 * loadFactor + (seedOffset % 5)))),
      network_bytes_ingress: Math.round(Math.max(1024 * 5, 8 * 1024 * 1024 * loadFactor)),
      network_bytes_egress: Math.round(Math.max(1024 * 20, 24 * 1024 * 1024 * loadFactor)),
    });
  }
...
async function getMonthlyPostgresCostFromSnapshots(tenantId: string, subscriptionIds: string[]): Promise<number> {
  ...
  const sql = `
    ...
    WHERE tenant_id = ?
      ...
      AND (
        LOWER(COALESCE(service_name, '')) LIKE '%postgres%'
        OR LOWER(COALESCE(service_name, '')) LIKE '%azure database for postgresql%'
      )
  `;
  ...
  try {
    ...
    console.warn(`[postgres-metrics] Snapshot cost fallback failed for ${tenantId}:`, error?.message);
    return 0;
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isMockTenant(tenantId) || tenantId.startsWith("mock-")) {
      const instances = [
        {
          id: "postgres-mock-prod",
          name: "postgres-prod-eastus",
          region: "East US",
          sku: "General Purpose (Standard_D4ds_v5)",
          monthlyCostUsd: 510,
          history: generateMockPostgresHistory(20),
        },
        {
          id: "postgres-mock-dev",
          name: "postgres-dev-eastus",
          region: "East US",
          sku: "Burstable (Standard_B2s)",
          monthlyCostUsd: 92,
          history: generateMockPostgresHistory(60),
        },
      ];
    ...
    const cacheKey = getDiagnosticsCacheKey("postgres-metrics", tenantId);
    ...
    if (totalCostFromCm <= 0 && resources.length > 0) {
      const fallbackTotal = await getMonthlyPostgresCostFromSnapshots(tenantId, subscriptionIds);
      if (fallbackTotal > 0) {
    ...
    const instances = await Promise.all(
      resources.map(async (resource) => {
        ...
        try {
          ...
          console.error(`[postgres-metrics] Error al consultar métricas para ${resource.name}:`, err.message);
          return {
            id: resource.id,
            name: resource.name,
            region: resource.location || "unknown",
    ...
    return NextResponse.json({
      mock: false,
      resourceExists: false,
      message: "No se pudieron consultar las métricas de PostgreSQL en este momento.",
      instances: [],
      errors: [{ code: "POSTGRES_METRICS_UNAVAILABLE", detail: error instanceof Error ? error.message : "Unknown error" }],
    });
  }
}
