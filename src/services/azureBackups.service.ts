    ...
    for (const vault of vaults) {
        ...
        if (orphanItems.length > 0) {
            ...
            if (orphanMonthlyCost > 1) {
                actions.push({
                    ...
                    titleKey: "rec_bkp_orphan_title",
                    descKey: "rec_bkp_orphan_desc",
                    impactKey: "rec_bkp_orphan_impact",
                    params: { vault: vault.name, count: orphanItems.length, gb: orphanStorageGb, savings: orphanMonthlyCost.toFixed(2) },
        ...
        if (vault.storageBreakdown.vaultStandardGB > 100) {
            ...
            if (savings > 5) {
                actions.push({
                    ...
                    vaultId: vault.id,
                    vaultName: vault.name,
                    titleKey: "rec_bkp_archive_title",
                    descKey: "rec_bkp_archive_desc",
                    impactKey: "rec_bkp_archive_impact",
                    params: { vault: vault.name, gb: Math.round(potentialArchiveGb), rate: storageRate, savings: savings.toFixed(2) },
        ...
        if (isNonProd && vault.redundancy === "GeoRedundant") {
            ...
            if (savings > 3) {
                actions.push({
                    ...
                    vaultId: vault.id,
                    vaultName: vault.name,
                    titleKey: "rec_bkp_redundancy_title",
                    descKey: "rec_bkp_redundancy_desc",
                    impactKey: "rec_bkp_redundancy_impact",
                    params: { vault: vault.name, env: vault.environmentTag.toUpperCase(), savings: savings.toFixed(2) },
        ...
        if (isNonProd && vault.asrProtectedItemsCount > 0) {
            ...
            actions.push({
                ...
                vaultId: vault.id,
                vaultName: vault.name,
                titleKey: "rec_bkp_asr_title",
                descKey: "rec_bkp_asr_desc",
                impactKey: "rec_bkp_asr_impact",
                params: { vault: vault.name, env: vault.environmentTag.toUpperCase(), count: vault.asrProtectedItemsCount, savings: savings.toFixed(2) },
