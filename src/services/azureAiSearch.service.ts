  ...
  for (const s of services) {
    ...
    if (s.isDevOrTest && (s.skuName === "Standard" || s.skuName === "Standard2" || s.skuName === "Standard3") && s.storageUsedGB < 2) {
      ...
      if (diff.gt(0)) {
        actions.push({
          ...
          serviceId: s.id,
          serviceName: s.name,
          params: { name: s.name, gb: s.storageUsedGB.toFixed(1), savings: diff.toFixed(2) },
    ...
    if (s.isOrphan) {
      actions.push({
        ...
        serviceId: s.id,
        serviceName: s.name,
        params: { name: s.name, cost: s.monthlyCostUSD },
    ...
    if (s.replicaCount > 1 && s.qpsAvg < 2) {
      ...
      if (savings.gt(0)) {
        actions.push({
          ...
          serviceId: s.id,
          serviceName: s.name,
          params: { name: s.name, replicas: s.replicaCount, qps: s.qpsAvg, savings: savings.toFixed(2) },
    ...
    if (s.semanticSearchTier === "free" && s.qpsAvg > 5) {
      actions.push({
        ...
        serviceId: s.id,
        serviceName: s.name,
        params: { name: s.name, qps: s.qpsAvg },
