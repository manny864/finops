  ...
  for (const alert of alerts) {
    ...
    if (alert.isOrphan && alert.isEnabled) {
      actions.push({
        ...
        ruleId: alert.id,
        ruleName: alert.name,
        title: `Eliminar regla huérfana '${alert.name}'`,
        description: `La regla sigue activa y consumiendo presupuesto mensual ($${alert.monthlyCostUSD.toFixed(2)}/mes) pero su recurso objetivo ('${alert.targetResourceName || alert.targetResourceId}') fue eliminado de Azure.`,
    ...
    if (
      ...
      actions.push({
        ...
        ruleId: alert.id,
        ruleName: alert.name,
        title: `Ajustar frecuencia de evaluación a 5m en '${alert.name}'`,
        description: `La regla de búsqueda de logs evalúa KQL cada 1 minuto en ambiente no productivo (${alert.resourceGroup}). Modificar el intervalo a 5 minutos reduce el costo de la regla en un 66% sin afectar la calidad operativa.`,
    ...
    if (
      ...
      actions.push({
        ...
        ruleId: alert.id,
        ruleName: alert.name,
        title: `Vincular Action Group a alerta silenciosa '${alert.name}'`,
        description: `La alerta (${alert.severity}) se encuentra habilitada pero no tiene ningún Action Group vinculado. Al dispararse, no notificará a los ingenieros de guardia ni ejecutará webhooks.`,
...
export function getMockAlertsPayload(tenantId: string): AlertsPayload {
  ...
  const alerts: AlertRuleResource[] = [
    // 1. High CPU Metric Alert
    {
      ...
      isInefficient: false,
      firingHistory: [
        { timestamp: new Date(Date.now() - 1000 * 60 * 45).toISOString(), status: "Firing", description: "CPU promedio alcanzó 92.4% (> 85.0%)" },
        { timestamp: new Date(Date.now() - 1000 * 60 * 60 * 6).toISOString(), status: "Resolved", description: "CPU normalizada a 44.1%" },
        { timestamp: new Date(Date.now() - 1000 * 60 * 60 * 7).toISOString(), status: "Firing", description: "Pico de tráfico CyberMonday CPU 89%" },
      ],
    },
    // 2. Scheduled Query Rule - High Error Rate KQL
    {
      ...
      isInefficient: false,
      firingHistory: [
        { timestamp: new Date(Date.now() - 1000 * 60 * 60 * 18).toISOString(), status: "Resolved", description: "Tasa de error estabilizada tras hotfix" },
        { timestamp: new Date(Date.now() - 1000 * 60 * 60 * 20).toISOString(), status: "Firing", description: "502 Bad Gateway detectados en gateway APIM" },
      ],
    },
