...
export function getMockNetworkingZombiesPayload(tenantId: string): NetworkingZombiesPayload {
  ...
  const zombies: NetworkZombieResourceItem[] = [
    {
      ...
      detectionReasonKey: "reason_vpn_no_tunnels_demo",
    ...
    },
    {
      ...
      detectionReasonKey: "reason_pip_no_nic_lb",
      monthlyCostUSD: 3.65,
      isExempted: false,
    },
    {
      ...
      detectionReasonKey: "reason_pip_vm_decommissioned",
      monthlyCostUSD: 3.65,
      isExempted: false,
    },
    {
      ...
      detectionReasonKey: "reason_nat_no_subnets_hourly",
    ...
    },
    {
      ...
      detectionReasonKey: "reason_pe_rejected_storage",
      monthlyCostUSD: 7.2,
      isExempted: false,
    },
    {
      ...
      detectionReasonKey: "reason_platform_watcher_default",
      monthlyCostUSD: 0.0,
      isExempted: true,
  ...
  if (isEnterprise) {
    zombies.push({
      ...
      detectionReasonKey: "reason_er_cold_standby",
...
export function assembleLiveNetworkingZombies(input: {
  ...
  for (const gw of rawGateways) {
    ...
    zombies.push({
      ...
      detectionReasonKey: "reason_gateway_no_tunnels",
      monthlyCostUSD: cost,
      isExempted,
  ...
  for (const pip of rawPublicIps) {
    ...
    zombies.push({
      ...
      detectionReasonKey: "reason_pip_orphan",
      monthlyCostUSD: 3.65,
      isExempted,
      exemptionReason: exemptions.get(lowerId)?.reason,
    });
  ...
  for (const nat of rawNatGateways) {
    ...
    zombies.push({
      ...
      detectionReasonKey: "reason_nat_no_subnets",
      monthlyCostUSD: 32.85,
      isExempted,
      exemptionReason: exemptions.get(lowerId)?.reason,
    });
  ...
  for (const pe of rawPrivateEndpoints) {
    ...
    if (isOrphan) {
      zombies.push({
        ...
        detectionReasonKey: "reason_pe_state",
        detectionReasonParams: { state },
        monthlyCostUSD: 7.2,
        isExempted,
        exemptionReason: exemptions.get(lowerId)?.reason,
      });
  ...
  for (const appgw of rawAppGateways) {
    ...
    zombies.push({
      ...
      detectionReasonKey: "reason_appgw_no_listeners",
  ...
  for (const nw of rawNetworkWatchers) {
    ...
    zombies.push({
      ...
      detectionReasonKey: "reason_platform_watcher_baseline",
      monthlyCostUSD: 0.0,
      isExempted: true,
