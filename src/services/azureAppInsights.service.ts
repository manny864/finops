  ...
  for (const item of items) {
    ...
    if (item.isDevOrTest && item.isDailyCapUnlimited && !item.isOrphan) {
      actions.push({
        ...
        params: { name: item.name, rg: item.resourceGroup },
    ...
    if (item.samplingPercentage === 100 && item.ingestedTotalGB > 5.0 && !item.isOrphan) {
      ...
      actions.push({
        ...
        params: {
          name: item.name,
          gb: item.ingestedTotalGB.toFixed(1),
          traces: (traceRatio * 100).toFixed(0),
    ...
    if (item.tracesGB > 10.0 && (item.tracesGB / (item.ingestedTotalGB || 1)) > 0.65 && !item.isOrphan) {
      ...
      actions.push({
        ...
        resourceId: item.id,
        resourceName: item.name,
        params: { name: item.name, gb: item.tracesGB.toFixed(1), savings: logSavings.toFixed(2) },
