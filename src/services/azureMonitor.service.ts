  ...
  for (const alert of alerts) {
    ...
    if (alert.alertType === "logSearch" && alert.dataProcessedGB_MTD >= 50 && alert.isEnabled && !alert.isOrphan) {
      ...
      if (potentialSavings > 30) {
        actions.push({
          ...
          params: { name: alert.name, gb: alert.dataProcessedGB_MTD.toFixed(1), cost: currentDataCost.toFixed(2), savings: potentialSavings.toFixed(2) },
    ...
    if (
      ...
      if (metricSavings > 15) {
        actions.push({
          ...
          resourceId: alert.id,
          resourceName: alert.name,
          params: { name: alert.name, savings: metricSavings.toFixed(2) },
    ...
    if (alert.isDevOrTest && alert.runFrequency === "1m" && alert.isEnabled && !alert.isOrphan) {
      ...
      actions.push({
        ...
        resourceId: alert.id,
        resourceName: alert.name,
        params: { name: alert.name, rg: alert.resourceGroup, savings: freqSavings.toFixed(2) },
    ...
    if (alert.isOrphan) {
      actions.push({
        ...
        resourceId: alert.id,
        resourceName: alert.name,
        params: { name: alert.name, target: alert.targetResourceName },
  ...
  const alerts: AzureAlertResource[] = [
    {
      ...
      primaryRecommendation: {
        category: "KQL_OPTIMIZE",
        params: { name: "alert-log-heavy-5xx", gb: "420.0", cost: "966.00", savings: "676.20" },
        savingsUSD: 676.20,
      },
    },
    {
      ...
      primaryRecommendation: {
        category: "MIGRATE_TO_METRIC",
        params: { name: "alert-cpu-perf-kql", savings: "195.45" },
        savingsUSD: 166.60,
      },
    },
    {
      ...
      primaryRecommendation: {
        category: "FREQUENCY_ADJUST",
        params: { name: "alert-dev-frequent-exceptions", rg: "rg-dev-backend", savings: "82.80" },
        savingsUSD: 63.00,
      },
    },
    {
    ...
    },
    {
      ...
      primaryRecommendation: {
        category: "ORPHAN_PURGE",
        params: { name: "alert-orphan-legacy-vm", target: "vm-deleted-legacy" },
        savingsUSD: 28.10,
      },
    },
  ];
