    ...
    for (const disk of disks) {
        ...
        if (disk.diskState === "Unattached") {
            ...
            actions.push({
                ...
                diskId: disk.id,
                diskName: disk.name,
                titleKey: "rec_disk_orphan_title",
                descKey: "rec_disk_orphan_desc",
                impactKey: "rec_disk_orphan_impact",
                params: { disk: disk.name, sizeGb: disk.diskSizeGB, savings: cost.toFixed(2) },
                category: "ORPHAN",
        ...
        if (isPremium && disk.diskState === "Attached" && (isDevOrTest || isLowIops)) {
            ...
            if (savings > 1) {
                actions.push({
                    ...
                    diskId: disk.id,
                    diskName: disk.name,
                    titleKey: "rec_disk_downgrade_title",
                    descKey: "rec_disk_downgrade_desc",
                    impactKey: "rec_disk_downgrade_impact",
                    params: { disk: disk.name, env: disk.environmentTag.toUpperCase(), iops: disk.metrics.avgIops, savings: savings.toFixed(2) },
        ...
        if (disk.vmPowerState === "deallocated" && isPremium) {
            ...
            if (savings > 2) {
                actions.push({
                    ...
                    diskId: disk.id,
                    diskName: disk.name,
                    titleKey: "rec_disk_deallocated_title",
                    descKey: "rec_disk_deallocated_desc",
                    impactKey: "rec_disk_deallocated_impact",
                    params: { vm: disk.managedByVmName || "VM", savings: savings.toFixed(2) },
