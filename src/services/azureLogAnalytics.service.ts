  ...
  for (const w of workspaces) {
    ...
    if (w.pricingTier === "PerGB2018" && w.avgDailyIngestionGB >= 100 && !w.isOrphan) {
      ...
      if (savings > 50) {
        actions.push({
          ...
          resourceId: w.id,
          resourceName: w.name,
          title: `Migrar '${w.name}' a Commitment Tier de ${targetTier.dailyGb} GB/día`,
          description: `El workspace ingiere un promedio de ${w.avgDailyIngestionGB.toFixed(1)} GB/día en Pay-As-You-Go ($${LAW_PAYG_RATE_PER_GB}/GB). Migrar a Capacity Reservation (${targetTier.dailyGb} GB a $${targetTier.pricePerGb}/GB) genera un ahorro estimado de ~$${savings.toFixed(2)} USD/mes (~15%).`,
    ...
    if (w.isDevOrTest && w.isDailyCapUnlimited && !w.isOrphan) {
      ...
      actions.push({
        ...
        resourceId: w.id,
        resourceName: w.name,
        title: `Fijar Límite Diario (Daily Cap) de 5 GB/día en '${w.name}'`,
        description: `El workspace se encuentra en entorno no productivo (${w.resourceGroup}) sin tope diario de ingesta. Fijar un límite de 5 GB/día previene facturas inesperadas ante bucles de logging descontrolados.`,
        category: "DAILY_CAP",
    ...
    if (w.retentionInDays > 90 && !w.isOrphan) {
      ...
      if (retentionSavings > 20) {
        actions.push({
          ...
          resourceId: w.id,
          resourceName: w.name,
          title: `Optimizar Retención de Datos en '${w.name}' (${w.retentionInDays} -> 30 días)`,
          description: `El workspace retiene logs activos durante ${w.retentionInDays} días en almacenamiento de alta velocidad. Reducir la retención interactiva a 30 días o migrar históricos a Data Archive ahorra ~$${retentionSavings.toFixed(2)} USD/mes en almacenamiento extendido.`,
    ...
    if (w.isOrphan) {
      actions.push({
        ...
        resourceId: w.id,
        resourceName: w.name,
        title: `Purga de Log Analytics Workspace Huérfano '${w.name}'`,
        description: `El workspace '${w.name}' no ha registrado ninguna ingesta de datos en los últimos 60 días. Se recomienda su desmantelamiento para eliminar costos residuales y limpiar la topología.`,
  ...
  const workspaces: LogAnalyticsResource[] = [
    {
      ...
      primaryRecommendation: {
        category: "COMMITMENT_TIER",
        title: "Migrar a Commitment Tier 100 GB/día",
        description: "Ahorro del 15% por volumen sostenido > 100 GB/día",
        savingsUSD: 1470.00,
      },
    },
    {
    ...
    },
    {
      ...
      primaryRecommendation: {
        ...
        description: "Evitar picos accidentales de logs en entorno Dev",
        savingsUSD: 124.20,
      },
    },
    {
      ...
      primaryRecommendation: {
        category: "RETENTION_ADJUST",
        title: "Reducir retención a 30 días",
        description: "Reducir retención de 365 a 30 días en staging",
        savingsUSD: 420.80,
      },
    },
    {
      ...
      primaryRecommendation: {
        category: "PURGE_ORPHAN",
        title: "Desmantelar workspace inactivo",
        description: "0 GB de ingesta durante los últimos 60 días",
        savingsUSD: 0,
      },
    },
  ];
