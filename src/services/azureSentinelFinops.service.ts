  ...
  for (const w of workspaces) {
    ...
    if (w.avgDailyIngestionGB >= 100 && w.lawPricingTier === "PerGB2018") {
      ...
      if (estSavings > 50) {
        actions.push({
          ...
          resourceId: w.id,
          resourceName: w.name,
          title: `Migrar '${w.name}' a Capacity Reservation (${suitableTier.dailyGb} GB/día)`,
          description: `El workspace ingiere ~${w.avgDailyIngestionGB.toFixed(1)} GB/día sostenido. El paso de Pay-As-You-Go a Capacity Reservation ${suitableTier.dailyGb} GB reduce el costo unitario consolidado de $4.30 a ~$${suitableTier.combinedPricePerGb.toFixed(2)} USD/GB, ahorrando ~$${estSavings.toLocaleString()} USD/mes (~25%).`,
    ...
    if (w.orphanRulesCount > 0) {
      ...
      actions.push({
        ...
        resourceId: w.id,
        resourceName: w.name,
        title: `Racionalizar ${w.orphanRulesCount} Reglas de Detección Inactivas en '${w.name}'`,
        description: `Se detectaron ${w.orphanRulesCount} reglas analíticas programadas de Sentinel sin alertas generadas en los últimos 90 días o que escanean tablas vacías, consumiendo ciclos de consulta KQL continuos.`,
    ...
    if (w.isDevOrTest && w.isDailyCapUnlimited) {
      actions.push({
        ...
        resourceId: w.id,
        resourceName: w.name,
        title: `Establecer Daily Cap de Seguridad en '${w.name}' (Dev/Test)`,
        description: `Workspace de seguridad en entorno no productivo sin límite diario de ingesta. Fijar un tope de 5 GB/día previene fugas financieras por bucles de auditoría o eventos desbordados.`,
        category: "DAILY_CAP",
    ...
    if (w.retentionInDays > 90) {
      ...
      if (estSavings > 20) {
        actions.push({
          ...
          resourceId: w.id,
          resourceName: w.name,
          title: `Migrar Retención Prolongada (>90d) a Log Archive en '${w.name}'`,
          description: `El workspace retiene logs interactivos por ${w.retentionInDays} días. Reducir la retención interactiva a 90 días y archivar los eventos históricos en Log Archive ($0.02 vs $0.12/GB-mes) genera un ahorro del 83% en almacenamiento.`,
  ...
  const workspaces: SentinelResource[] = [
    {
      ...
      primaryRecommendation: {
        category: "COMMITMENT_TIER",
        title: "Migrar a Capacity Reservation (100 GB/día)",
        description: "Ahorro de ~25% en tarifa combinada Sentinel + LAW",
        savingsUSD: 3950.0,
      },
    },
    {
      ...
      primaryRecommendation: {
        ...
        description: "Prevenir desbordes de ingesta en laboratorio",
        savingsUSD: 387.0,
      },
    },
    {
      ...
      primaryRecommendation: {
        category: "RETENTION_ADJUST",
        title: "Migrar retención >90d a Data Archive",
        description: "Reducción de costo de almacenamiento a $0.02/GB-mes",
        savingsUSD: 680.0,
      },
    },
  ];
