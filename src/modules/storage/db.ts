...
export async function initializeDatabase() {
    ...
    dbInitPromise = (async () => {
        ...
        // Filas de costo por ResourceType, para el desglose por categoría FinOps
        // (join a OpenDataServices.resource_type → service_category). Tabla
        // dedicada para no alterar la grain de CostSnapshots.
        await connection.query(`
...
/**
 * Inserts (or replaces) a cost row aggregated by ResourceType into
 * CostCategorySnapshots. Idempotente via UNIQUE KEY (tenant, sub, date,
 * resource_type). El resource_type se une luego a OpenDataServices para
 * derivar la categoría FinOps.
 */
