        ...
        <div className="w-full max-w-full space-y-6 animate-in fade-in duration-200">
            ...
            <div className="border border-rose-200 dark:border-rose-900/50 bg-rose-50/20 dark:bg-rose-950/10 p-6 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
                ...
                >
                    ...
                    <span>{billingData?.cancelAtPeriodEnd ? t("cancellationScheduled") : t("cancelSubscription")}</span>
                </button>
            </div>
                                        ...
                                        {columns.find((c) => c.id === "actions")?.visible && (
                                            <td className="px-4 py-3 whitespace-nowrap">
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                        // La ruta pide tenantId y redacta el PDF en el
                                                        // idioma activo; sin los dos parametros devolvia 400.
                                                        const url = inv.downloadPdfUrl
                                                            || `/api/billing/invoices/${inv.id}/pdf?tenantId=${encodeURIComponent(tenantId)}&locale=${encodeURIComponent(locale)}`;
