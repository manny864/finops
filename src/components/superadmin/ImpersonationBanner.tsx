    ...
    >
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <p className="truncate leading-tight">
          <strong className="text-white font-bold mr-1.5">Modo Impersonación Activo:</strong>
          <span className="text-slate-300">
            Estás navegando como Administrador dentro de{" "}
            <span className="text-blue-400 font-semibold">{session.targetTenantName}</span>{" "}
            <span className="text-slate-400 font-mono text-[11px]">({session.targetTenantId})</span>.
          </span>
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        >
          ...
          <span>{isStopping ? "Saliendo..." : "Salir de la sesión de cliente"}</span>
        </button>
      </div>
    </div>
  );
