...
export default function ComputeServiceCostBoard({
    family,
    title,
    subtitle,
    icon,
}: {
    family: "functions" | "batch" | "aro";
    title: string;
    subtitle: string;
    icon?: React.ReactNode;
}) {
    const t = useTranslations("ComputeFamilies");
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const url = new URL("/api/intelligence/compute/service-cost", window.location.origin);
    ...
    if (loading) {
        return <div className="p-6 max-w-5xl mx-auto text-gray-500">{t("loading")}</div>;
    }
