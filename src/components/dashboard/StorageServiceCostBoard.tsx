...
export default function StorageServiceCostBoard({
    family,
    title,
    subtitle,
    icon,
}: {
    family: "managed-disks" | "backups" | "data-lake-gen2";
    title: string;
    subtitle: string;
    icon?: React.ReactNode;
}) {
    const t = useTranslations("StorageFamilies");
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const url = new URL("/api/intelligence/storage/service-cost", window.location.origin);
