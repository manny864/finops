...
import { buildAmlRemediationCommand } from "@/lib/aiRemediations";

// ─── Paleta de colores en tonos de azul ───
const CATEGORY_COLORS: Record<string, string> = {
  AUTO_SHUTDOWN_CI: "#0078D4",
  SCALE_TO_ZERO_CLUSTER: "#2563EB",
  SPOT_TRAINING: "#0284C7",
...
function RemediationModal({
  ...
  action: AmlRemediationAction | null;
  onClose: () => void;
}) {
  ...
  const commands = buildAmlRemediationCommand(action);
...
export default function AMLDashboard() {
  ...
  const [sortKey, setSortKey] = useState<keyof AmlComputeResource>("monthlyCostUSD");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(15);
  const [activeModalAction, setActiveModalAction] = useState<AmlRemediationAction | null>(null);
  ...
  const apiUrl = canFetch
    ? `/api/intelligence/azure-ai/aml?tenantId=${encodeURIComponent(tenantId!)}&timeRange=${timeRange}${isDemo ? "&mock=true" : ""}`
    : null;

  const { data, error, isLoading, mutate } = useSWR<AmlPayload>(
    apiUrl,
    fetcher,
  ...
  // ─── Opciones de Filtro ───
  const typeOptions = ["ALL", ...new Set(resources.map((r) => r.computeType))];
  ...
  const filteredResources = resources
    .filter((r) => filterType === "ALL" || r.computeType === filterType)
  ...
  const handleSort = (key: keyof AmlComputeResource) => {
    if (sortKey === key) {
  ...
  const SortIcon = ({ column }: { column: keyof AmlComputeResource }) => {
  ...
  // ─── Datos para Donut ───
  const donutData = summary.breakdownByComputeType.map((m) => ({
    name: m.type,
    value: m.costUSD,
    color: m.color,
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ─── Fila 1: Gráficas de Capacidad & Utilización ─── */}
          ...
          <table className="w-full text-xs">
            <thead className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30">
              <tr>
                ...
                <ResizableTh minWidth={130}>
                  <button
                    className="flex items-center gap-1 cursor-pointer hover:text-[#0078D4]"
                    onClick={() => handleSort("avgCpuPercentage")}
                  >
                    % CPU Promedio
                    <SortIcon column="avgCpuPercentage" />
                  </button>
                </ResizableTh>
                <ResizableTh minWidth={130}>
                  <button
            ...
            <tbody>
              {paginatedResources.map((resource) => {
                const isCI = resource.computeType === "ComputeInstance";
                const isCluster = resource.computeType === "AmlComputeCluster";
                const Icon = isCI ? IconTerminal2 : isCluster ? IconServer : IconCpu;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-bold text-[#1B2A41] dark:text-slate-200">
                      {format(resource.monthlyCostUSD)}
                    </td>
                    <td className="px-4 py-3">
                      {matchingAction ? (
                        <button
