...
export default function AppServiceRightsizingTab() {
    ...
    if (isLoading) return (
        <div className="flex items-center justify-center py-16">
            ...
            <span className="text-gray-500">Cargando recomendaciones App Service...</span>
        </div>
    );
    ...
    if (error) {
        ...
        if (requiredTier) {
            return <TierLockedNotice requiredTier={requiredTier} currentTier={(selectedTenant as any)?.tier} featureName="Rightsizing de App Service" />;
        }
        return (
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-100 dark:border-blue-900/40">
                    <p className="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wide">Planes Analizados</p>
                ...
                <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-4 border border-orange-100 dark:border-orange-900/40">
                    <p className="text-xs text-orange-600 dark:text-orange-400 font-semibold uppercase tracking-wide">CPU Prom. Máx.</p>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 dark:bg-slate-800/50 text-xs text-slate-500 dark:text-slate-400">
                        <tr>
                            ...
                            <th className="px-4 py-3 font-semibold">SKU Recom.</th>
                            <th className="px-4 py-3 font-semibold text-center">CPU Prom.</th>
                            <th className="px-4 py-3 font-semibold text-center">MEM Prom.</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-800/50">
                        ...
                        {filtered.length === 0 && (
                            <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-slate-500">No hay recomendaciones App Service.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
