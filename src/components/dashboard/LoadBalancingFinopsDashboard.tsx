...
export default function LoadBalancingFinopsDashboard() {
    ...
    // Modals
    const [selectedResource, setSelectedResource] = useState<LoadBalancingResource | null>(null);
    const [activeRemediation, setActiveRemediation] = useState<LoadBalancingRemediationAction | null>(null);
    ...
    const fetcher = async (url: string) => {
        ...
        return res.json() as Promise<LoadBalancingResponse>;
    };

    const apiUrl = tenantId && tenantId !== "default"
        ? `/api/intelligence/network/load-balancing?tenantId=${encodeURIComponent(tenantId)}`
        : `/api/intelligence/network/load-balancing?tenantId=demo-tenant-id`;

    const { data, error, isLoading, mutate } = useSWR<LoadBalancingResponse>(apiUrl, fetcher, {
        ...
        return resources
            .filter((res) => {
                ...
                if (searchQuery.trim() !== "") {
                    ...
                    const matchIp = res.publicIpOrFqdn.toLowerCase().includes(q);
    ...
    // Service Icon Helper
    const getServiceIcon = (svc: LoadBalancingServiceType) => {
        switch (svc) {
            case "Application Gateway":
                return <IconScale className="w-4 h-4 text-[#0078D4] shrink-0" stroke={1.5} />;
            case "Front Door":
                return <IconDoor className="w-4 h-4 text-[#2563EB] shrink-0" stroke={1.5} />;
            case "Load Balancer":
                return <IconSwitchHorizontal className="w-4 h-4 text-[#0284C7] shrink-0" stroke={1.5} />;
            case "Traffic Manager":
                return <IconGlobe className="w-4 h-4 text-[#38BDF8] shrink-0" stroke={1.5} />;
            default:
                return <IconScale className="w-4 h-4 text-[#0078D4] shrink-0" stroke={1.5} />;
        }
    };
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2">
                        <IconScale className="w-7 h-7 text-[#0054A6]" stroke={1.5} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {/* Card 2: Balanceadores & Ingress */}
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1.5">
                                <IconScale className="w-4 h-4 text-[#2563EB]" stroke={1.5} />
                                {t("kpiGatewaysTitle")}
                            </span>
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl sm:text-3xl font-extrabold text-[#1B2A41] dark:text-white">
                                {(kpis?.totalGatewaysCount || 0) + (kpis?.totalFrontDoorsCount || 0) + (kpis?.totalLoadBalancersCount || 0) + (kpis?.totalTrafficManagersCount || 0)}
                            </span>
                            <span className="text-xs text-slate-400 font-medium">{t("ingressUnits")}</span>
                        </div>
                    </div>
                    <div className="mt-3 pt-2 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                        <span>{kpis?.totalGatewaysCount || 0} AppGWs · {kpis?.totalFrontDoorsCount || 0} AFD</span>
                        <span>{kpis?.totalLoadBalancersCount || 0} LBs · {kpis?.totalTrafficManagersCount || 0} TM</span>
                    </div>
                </div>

                {/* Card 3: Fugas / Huérfanos */}
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1.5">
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl sm:text-3xl font-extrabold text-[#1B2A41] dark:text-white">
                                {kpis?.orphanedCount || 0}
                            </span>
                            {(kpis?.orphanedCount || 0) > 0 ? (
                ...
                {/* Card 4: Tráfico / Solicitudes */}
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1.5">
                                <IconActivity className="w-4 h-4 text-[#0284C7]" stroke={1.5} />
                                {t("kpiRequestsTitle")}
                            </span>
                            <InfoTooltip content={t("kpiRequestsTooltip")} position="bottom" align="right" />
                        </div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl sm:text-3xl font-extrabold text-[#1B2A41] dark:text-white">
                                {kpis?.totalRequestsMillion || 0}M
                            </span>
                            <span className="text-xs text-slate-400 font-medium">Requests MTD</span>
                        </div>
                    </div>
                    <div className="mt-3 pt-2 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                            ...
                            <PieChart>
                                ...
                                >
                                    {(kpis?.breakdown || []).map((entry, idx) => (
                                        <Cell
                                            key={`cell-${idx}`}
                                            fill={LOAD_BALANCING_COLORS[entry.serviceName] || "#0078D4"}
                                            stroke="transparent"
                                        />
                                    ))}
                                </Pie>
                            ...
                            >
                                <div className="flex items-center gap-2.5">
                                    <span
                                        ...
                                        style={{ backgroundColor: LOAD_BALANCING_COLORS[item.serviceName] || "#0078D4" }}
                                    />
                            ...
                            {paginatedResources.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="py-8 text-center text-slate-500 dark:text-slate-400">
                                        <IconScale className="w-8 h-8 mx-auto text-slate-300 dark:text-slate-600 mb-2" stroke={1.5} />
                                    ...
                                    >
                                        ...
                                        <td className="py-3 px-3.5 font-medium text-[#1B2A41] dark:text-slate-100">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                {res.wafEnabled && (
                                                    <span className="px-1.5 py-0.5 text-[9px] font-bold rounded-md bg-blue-50 dark:bg-blue-950/40 text-[#0054A6] dark:text-sky-400 border border-blue-200 dark:border-blue-800 shrink-0">
                                                        WAF
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <td className="py-3 px-3.5">
                                            {res.operationalState === "Healthy" || res.operationalState === "Running" ? (
                                            ...
                                            ) : res.operationalState === "Orphan" ? (
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-900">
                                                ...
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                                                    {res.operationalState}
                                                </span>
                                            )}
                                        </td>

                                        <td className="py-3 px-3.5 text-slate-600 dark:text-slate-400 font-mono text-[11px] truncate" title={res.publicIpOrFqdn}>
                                            {res.publicIpOrFqdn}
                                        </td>
                                    ...
                                    <div className="flex items-start justify-between gap-3 mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className="p-1.5 rounded-lg bg-blue-50 dark:bg-blue-950/50 text-[#0078D4]">
                                                {rem.category === "ORPHAN_LB" && <IconSwitchHorizontal className="w-4 h-4" stroke={1.5} />}
                                                {rem.category === "FRONTDOOR_SKU_DOWNGRADE" && <IconDoor className="w-4 h-4" stroke={1.5} />}
                                                {rem.category === "APP_GATEWAY_AUTOSCALE" && <IconScale className="w-4 h-4" stroke={1.5} />}
            ...
            {/* Ingress Details Modal (Strict Z-Index Layering: z-50 / z-[100]) */}
            {selectedResource && (
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-4 z-50">
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs">
                            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/60">
                                <span className="text-slate-400 block mb-0.5">{t("skuCapacityLabel")}</span>
                            ...
                            <div className="space-y-3 pt-2">
                                ...
                                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 text-xs space-y-1.5">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-semibold">{selectedResource.wafEnabled ? t("wafEnabled") : t("wafDisabled")}</span>
                                    </div>
