...
export default function VmssRightsizingTab() {
    ...
    if (isLoading) return (
        <div className="flex items-center justify-center py-16">
            ...
            <span className="text-gray-500">Cargando recomendaciones VMSS...</span>
        </div>
    );
    ...
    if (error) {
        ...
        if (requiredTier) {
            return <TierLockedNotice requiredTier={requiredTier} currentTier={(selectedTenant as any)?.tier} featureName="Rightsizing de VMSS" />;
        }
        return (
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-100 dark:border-blue-900/40">
                    <p className="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-wide">VMSS Analizados</p>
                ...
                <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl p-4 border border-emerald-100 dark:border-emerald-900/40">
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold uppercase tracking-wide">Ahorro Estimado/mes</p>
                ...
                <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 border border-purple-100 dark:border-purple-900/40">
                    <p className="text-xs text-purple-600 dark:text-purple-400 font-semibold uppercase tracking-wide">Con Autoscale</p>
            ...
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-3">
                <input
                    ...
                    placeholder="Buscar VMSS, RG, SKU..."
            ...
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-50 dark:bg-slate-800/50 text-xs text-slate-500 dark:text-slate-400">
                        <tr>
                            <th className="px-4 py-3 font-semibold">VMSS</th>
                            <th className="px-4 py-3 font-semibold">SKU Actual</th>
                            <th className="px-4 py-3 font-semibold text-center">Cap. Actual</th>
                            <th className="px-4 py-3 font-semibold text-center">Cap. Recom.</th>
                            <th className="px-4 py-3 font-semibold text-center">CPU Prom.</th>
                            <th className="px-4 py-3 font-semibold text-center">Autoscale</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-800/50">
                        {pagedFiltered.map((r, i) => (
                            <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                                ...
                                <td className="px-4 py-3 text-center text-xs">
                                    {r.hasAutoscale
                                        ? <span className="px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full font-semibold">Sí</span>
                                        : <span className="px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-full font-semibold">No</span>}
                                </td>
                        ...
                        {filtered.length === 0 && (
                            <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-slate-500">No hay recomendaciones VMSS.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
