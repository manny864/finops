...
const CATEGORY_COLORS: Record<string, string> = {
  MIGRATE_TO_JOBS: "#0078D4",
  REDUCE_AUTOTERMINATION: "#2563EB",
  SINGLE_NODE_DEV: "#0284C7",
};
...
function RemediationModal({
  ...
  action: DatabricksRemediationAction | null;
  onClose: () => void;
}) {
  const [copied, setCopied] = useState(false);
  const [cmdTab, setCmdTab] = useState<"cli" | "powershell">("cli");
  const t = useTranslations("AzureAI");
  const textoRem = useTextoPorCategoria("AzureAI", "DBX");
  const { format } = useCurrency();

  if (!action) return null;

  const commands = buildDatabricksRemediationCommand(action);
...
export default function DatabricksDashboard() {
  ...
  const [sortKey, setSortKey] = useState<keyof DatabricksClusterItem>("monthlyCostUSD");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(15);
  const [activeModalAction, setActiveModalAction] = useState<DatabricksRemediationAction | null>(null);
  ...
  const apiUrl = canFetch
    ? `/api/intelligence/azure-ai/databricks?tenantId=${encodeURIComponent(tenantId!)}&timeRange=${timeRange}${isDemo ? "&mock=true" : ""}`
    : null;

  const { data, error, isLoading, mutate } = useSWR<DatabricksPayload>(
    apiUrl,
    fetcher,
  ...
  if (!isDemo && (!data || data.workspaces.length === 0) && !isLoading) {
  ...
  const { summary, workspaces, clusters, dailyTrend, remediationActions } = data;

  // ─── Opciones de Filtro ───
  const typeOptions = ["ALL", ...new Set(clusters.map((c) => c.computeType))];
  const rgOptions = ["ALL", ...new Set(clusters.map((c) => c.resourceGroup))];
  const subOptions = ["ALL", ...new Set(clusters.map((c) => c.subscriptionName))];

  // ─── Filtrado ───
  const filteredClusters = clusters
    .filter((c) => filterType === "ALL" || c.computeType === filterType)
    .filter((c) => filterResourceGroup === "ALL" || c.resourceGroup === filterResourceGroup)
    .filter((c) => filterSubscription === "ALL" || c.subscriptionName === filterSubscription)
    .filter((c) => {
      ...
      return (
        ...
        c.nodeType.toLowerCase().includes(q)
      );
    });
  ...
  const paginatedClusters = sortedClusters.slice((page - 1) * pageSize, page * pageSize);

  const handleSort = (key: keyof DatabricksClusterItem) => {
    if (sortKey === key) {
  ...
  const SortIcon = ({ column }: { column: keyof DatabricksClusterItem }) => {
  ...
  // ─── Datos para Donut ───
  const donutData = summary.breakdownByComponent.map((m) => ({
    name: m.component,
    value: m.costUSD,
    color: m.color,
  }));
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs p-5">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={dailyTrend}>
              <defs>
                <linearGradient id="dbuGrad" x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="px-5 py-3 flex items-center gap-3 flex-wrap border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="text-xs border border-slate-200 dark:border-slate-700 rounded-lg pl-8 pr-2.5 py-1 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 w-48"
            />
          </div>
          ...
          <span className="text-[11px] text-slate-400 ml-auto">
            {t("dbx_clusters_in_workspaces", { clusters: filteredClusters.length, workspaces: workspaces.length })}
          </span>
        </div>
          ...
          <table className="w-full text-xs">
            <thead className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30">
              <tr>
                ...
                <ResizableTh minWidth={130}>
                  <button
                    className="flex items-center gap-1 cursor-pointer hover:text-[#0078D4]"
                    onClick={() => handleSort("autoterminationMinutes")}
                  >
                    Auto-Termination
                    <SortIcon column="autoterminationMinutes" />
                  </button>
                </ResizableTh>
                <ResizableTh minWidth={120}>
                  <button
                    className="flex items-center gap-1 cursor-pointer hover:text-[#0078D4]"
                    onClick={() => handleSort("dbuRatePerHour")}
                  >
                    Tarifa DBU/h
                    <SortIcon column="dbuRatePerHour" />
                  </button>
                </ResizableTh>
                <ResizableTh minWidth={130}>
                  <button
            ...
            <tbody>
              {paginatedClusters.map((cluster) => {
                const isJob = cluster.computeType === "Job";
                const isServerless = cluster.computeType === "ServerlessSQL";
                const Icon = isJob ? IconCpu : isServerless ? IconDatabase : IconServer;

                const matchingAction = remediationActions.find(
                  (a) => a.targetId === cluster.clusterId
                );
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-400">{cluster.workspaceName}</span>
                        </div>
                        <span
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400 font-mono text-[11px] max-w-[180px] truncate">
                      {cluster.nodeType} ({cluster.minWorkers} - {cluster.maxWorkers} workers)
                    </td>
                    <td className="px-4 py-3">
                      {cluster.autoterminationMinutes === 0 ? (
                        ...
                        <span className="text-[10px] font-bold bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded">
                          {cluster.autoterminationMinutes} min
                        </span>
                      ) : (
