...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  BLOCKLIST_OPTIMIZATION: "#0284C7",
};

const CATEGORY_LABELS: Record<string, string> = {
  HASH_CACHING: "Caché de Contenido (SHA-256)",
  DEV_F0_DOWNGRADE: "Downgrade a Free Tier F0",
  BLOCKLIST_OPTIMIZATION: "Racionalización de Blocklists",
};
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-b from-slate-50/80 to-white dark:from-slate-900/80 dark:to-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-8 text-center shadow-xs">
        ...
        <h4 className="text-base font-bold text-[#1B2A41] dark:text-slate-100 mb-1.5">
          {t("empty_title") || "Sin telemetría de moderación registrada"}
        </h4>
        <p className="text-xs text-slate-500 dark:text-slate-400 max-w-lg mx-auto leading-relaxed">
          {t("empty_description") ||
            "No se detectaron cuentas de Azure AI Content Safety con actividad evaluable en este ciclo."}
        </p>
            ...
            {[
              {
                ...
                desc: "Aprovisiona un recurso Microsoft.CognitiveServices/accounts con kind ContentSafety.",
              },
              {
              ...
              },
              {
                ...
                desc: "Las solicitudes de análisis de texto e imagen se reflejarán automáticamente.",
              },
            ].map((step, idx) => (
              <div
                key={idx}
...
function RemediationModal({
  ...
  action: ContentSafetyRemediationAction | null;
  onClose: () => void;
}) {
  ...
  const commands = buildContentSafetyRemediationCommand(action);
...
export default function ContentSafetyDashboard() {
  ...
  const [sortKey, setSortKey] = useState<keyof ContentSafetyResource>("totalCostUSD");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(15);
  const [activeModalAction, setActiveModalAction] = useState<ContentSafetyRemediationAction | null>(null);
  ...
  const apiUrl = canFetch
    ? `/api/intelligence/azure-ai/content-safety?tenantId=${encodeURIComponent(tenantId!)}&timeRange=${timeRange}${isDemo ? "&mock=true" : ""}`
    : null;

  const { data, error, isLoading, mutate } = useSWR<ContentSafetyPayload>(
    apiUrl,
    fetcher,
  ...
  const handleSort = (key: keyof ContentSafetyResource) => {
    if (sortKey === key) {
  ...
  const SortIcon = ({ column }: { column: keyof ContentSafetyResource }) => {
  ...
  // ─── Datos para Donut ───
  const donutData = summary.breakdownByModality.map((m) => ({
    name: m.modality,
    value: m.costUSD,
    color: m.color,
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <KpiCard
          ...
          label="Textos Moderados"
          value={`${(summary.totalTextRecords / 1_000_000).toFixed(1)}M`}
          sub={`${summary.totalTextRecords.toLocaleString()} registros de texto`}
        />
        <KpiCard
          icon={IconPhoto}
      ...
      {/* ─── Fila 1: Gráficas de Capacidad & Detección ─── */}
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs overflow-hidden">
        ...
        <div className="px-5 py-3 flex items-center gap-3 flex-wrap border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30">
          ...
          <span className="text-[11px] text-slate-400 ml-auto">
            {t("cs_moderation_resources_count", { n: filteredResources.length })}
          </span>
        </div>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <IconShieldCheck className="w-4 h-4 text-[#0078D4] shrink-0" stroke={1.5} />
                        <div>
                    ...
                    <td className="px-4 py-3 font-mono text-slate-600 dark:text-slate-400">
                      {(resource.textRecordsCount / 1000).toFixed(1)}K
                    </td>
                    <td className="px-4 py-3 font-mono text-slate-600 dark:text-slate-400">
                      {resource.imagesAnalyzedCount > 0
                        ? `${(resource.imagesAnalyzedCount / 1000).toFixed(1)}K`
                        : "—"}
                    </td>
                    <td className="px-4 py-3 font-mono text-slate-600 dark:text-slate-400">
                      {resource.blocklistMatchesCount > 0
                        ? resource.blocklistMatchesCount.toLocaleString()
                        : "—"}
                    </td>
