...
export default function InternetAccessFinopsDashboard() {
    ...
    // Modals
    const [selectedResource, setSelectedResource] = useState<InternetAccessResource | null>(null);
    const [activeRemediation, setActiveRemediation] = useState<InternetAccessRemediationAction | null>(null);
    ...
    const fetcher = async (url: string) => {
        ...
        return res.json() as Promise<InternetAccessResponse>;
    };

    const apiUrl = tenantId && tenantId !== "default"
        ? `/api/intelligence/network/internet-access?tenantId=${encodeURIComponent(tenantId)}`
        : `/api/intelligence/network/internet-access?tenantId=demo-tenant-id`;

    const { data, error, isLoading, mutate } = useSWR<InternetAccessResponse>(apiUrl, fetcher, {
        ...
        return resources
            .filter((res) => {
                ...
                if (searchQuery.trim() !== "") {
                    ...
                    const matchIp = res.ipAddressOrPrefix.toLowerCase().includes(q);
    ...
    // Service Icon Helper
    const getServiceIcon = (svc: InternetAccessServiceType) => {
        switch (svc) {
            case "Azure Firewall":
                return <IconFlame className="w-4 h-4 text-[#0078D4] shrink-0" stroke={1.5} />;
            case "DDoS Protection":
                return <IconShieldCheck className="w-4 h-4 text-[#2563EB] shrink-0" stroke={1.5} />;
            case "NAT Gateway":
                return <IconArrowsSplit className="w-4 h-4 text-[#0284C7] shrink-0" stroke={1.5} />;
            case "Public IP":
                return <IconWorld className="w-4 h-4 text-[#38BDF8] shrink-0" stroke={1.5} />;
            default:
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2">
                        <IconWorld className="w-7 h-7 text-[#0054A6]" stroke={1.5} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {/* Card 2: Recursos Perimetrales */}
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1.5">
                                <IconLayersLinked className="w-4 h-4 text-[#2563EB]" stroke={1.5} />
                                {t("kpiPerimeterTitle")}
                            </span>
                            <InfoTooltip content={t("kpiPerimeterTooltip")} position="bottom" align="right" />
                        </div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl sm:text-3xl font-extrabold text-[#1B2A41] dark:text-white">
                                {(kpis?.totalPublicIpsCount || 0) + (kpis?.totalNatGatewaysCount || 0) + (kpis?.totalFirewallsCount || 0) + (kpis?.totalDdosPlansCount || 0)}
                            </span>
                            <span className="text-xs text-slate-400 font-medium">{t("perimeterUnits")}</span>
                        </div>
                    </div>
                    <div className="mt-3 pt-2 border-t border-slate-100 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
                        <span>{kpis?.totalPublicIpsCount || 0} IPs · {kpis?.totalNatGatewaysCount || 0} NAT GWs</span>
                        <span>{kpis?.totalFirewallsCount || 0} FW · {kpis?.totalDdosPlansCount || 0} DDoS</span>
                    </div>
                </div>

                {/* Card 3: IPs Huérfanas / Fugas */}
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1.5">
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl sm:text-3xl font-extrabold text-[#1B2A41] dark:text-white">
                                {kpis?.orphanedIpsCount || 0}
                            </span>
                            {(kpis?.orphanedIpsCount || 0) > 0 ? (
                ...
                {/* Card 4: Gasto en Seguridad Perimetral */}
                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 p-5 flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">
                            <span className="flex items-center gap-1.5">
                                <IconLock className="w-4 h-4 text-[#0284C7]" stroke={1.5} />
                                {t("kpiSecurityCostTitle")}
                            </span>
                            <InfoTooltip content={t("kpiSecurityCostTooltip")} position="bottom" align="right" />
                        </div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-2xl sm:text-3xl font-extrabold text-[#1B2A41] dark:text-white">
                                {format(kpis?.perimeterSecurityCostUSD || 0)}
                            </span>
                            <span className="text-xs text-slate-400 font-medium">MTD</span>
                        </div>
                    </div>
                            ...
                            <PieChart>
                                ...
                                >
                                    {(kpis?.breakdown || []).map((entry, idx) => (
                                        <Cell
                                            key={`cell-${idx}`}
                                            fill={INTERNET_ACCESS_COLORS[entry.serviceName] || "#0078D4"}
                                            stroke="transparent"
                                        />
                                    ))}
                                </Pie>
                            ...
                            >
                                <div className="flex items-center gap-2.5">
                                    <span
                                        ...
                                        style={{ backgroundColor: INTERNET_ACCESS_COLORS[item.serviceName] || "#0078D4" }}
                                    />
                            ...
                            {paginatedResources.length === 0 ? (
                                <tr>
                                    <td colSpan={9} className="py-8 text-center text-slate-500 dark:text-slate-400">
                                        <IconWorld className="w-8 h-8 mx-auto text-slate-300 dark:text-slate-600 mb-2" stroke={1.5} />
                                ...
                                paginatedResources.map((res) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-3 px-3.5 text-slate-600 dark:text-slate-400 font-mono text-[11px] truncate" title={res.ipAddressOrPrefix}>
                                            {res.ipAddressOrPrefix}
                                        </td>

                                        <td className="py-3 px-3.5">
                                            {res.associationStatus === "Attached" || res.associationStatus === "Active" ? (
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900">
                                                    ...
                                                    {res.associationStatus === "Attached" ? t("statusAttached") : t("statusActive")}
                                                </span>
                                            ) : res.associationStatus === "Unattached" ? (
                                    ...
                                    <div className="flex items-start justify-between gap-3 mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className="p-1.5 rounded-lg bg-blue-50 dark:bg-blue-950/50 text-[#0078D4]">
                                                {rem.category === "ORPHAN_IP" && <IconWorld className="w-4 h-4" stroke={1.5} />}
                                                {rem.category === "DDOS_ARBITRAGE" && <IconShieldCheck className="w-4 h-4" stroke={1.5} />}
                                                {rem.category === "FIREWALL_RIGHTSIZING" && <IconFlame className="w-4 h-4" stroke={1.5} />}
            ...
            {/* Resource Details Modal (Strict Z-Index Layering: z-50 / z-[100]) */}
            {selectedResource && (
