...
function request(qs = '', headers: Record<string, string> = { authorization: `Bearer ${CRON_SECRET}` }): NextRequest {
    return new NextRequest(`http://localhost/api/cron/prewarm-daily${qs}`, {
        headers,
    });
}

describe('GET / POST /api/cron/prewarm-daily — Job Diario de Pre-cálculo', () => {
    beforeEach(() => {
        vi.resetModules();
        fakeRedis = makeFakeRedis();
        process.env.CRON_SECRET = CRON_SECRET;
        // Mock global fetch for self-fetches
    ...
    it('rechaza llamadas sin autenticación válida', async () => {
        const { GET } = await import('@/app/api/cron/prewarm-daily/route');
        const res = await GET(request('', { authorization: 'Bearer invalid-token' }));
        expect(res.status).toBe(401);
    });

    it('inicia el prewarm en background y responde 202 con statusPollUrl', async () => {
        const { POST } = await import('@/app/api/cron/prewarm-daily/route');
        const res = await POST(request());
        const json = await res.json();

        expect(res.status).toBe(202);
        expect(json.message).toContain('Prewarm daily job started');
        expect(json.statusPollUrl).toBe('/api/cron/prewarm-daily?status=1');
