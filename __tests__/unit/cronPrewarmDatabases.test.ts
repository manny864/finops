...
function request(qs = '', headers: Record<string, string> = { authorization: `Bearer ${CRON_SECRET}` }): NextRequest {
    return new NextRequest(`http://localhost/api/cron/prewarm-databases${qs}`, {
        headers,
    });
}

describe('GET / POST /api/cron/prewarm-databases — Job de Pre-cálculo de Bases de Datos', () => {
    beforeEach(() => {
        vi.resetModules();
    ...
    it('rechaza llamadas sin autenticación válida', async () => {
        const { GET } = await import('@/app/api/cron/prewarm-databases/route');
        const res = await GET(request('', { authorization: 'Bearer invalid-token' }));
        expect(res.status).toBe(401);
    });

    it('inicia el prewarm de bases de datos en background y responde 202 con statusPollUrl', async () => {
        const { POST } = await import('@/app/api/cron/prewarm-databases/route');
        const res = await POST(request());
        const json = await res.json();

        expect(res.status).toBe(202);
        expect(json.message).toContain('Prewarm databases job started');
        expect(json.statusPollUrl).toBe('/api/cron/prewarm-databases?status=1');
