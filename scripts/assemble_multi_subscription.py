...
def main():
    ...
    print("Iniciando actualización Multi-Suscripción...")

    # 1. Update SOP
    sop_path = os.path.join(base_dir, "directivas", "azure_resourcegraph_SOP.md")
...
export default function ZombieResourcesTable() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Petición al Backend SIN subscriptionId para forzar escaneo global
        const res = await fetch(`/api/recommendations?tenantId=${tenantId}`, {
            headers: {
        ...
        const formattedDisks = (json.unattachedDisks || []).map((d: any) => ({
            ...
            subscriptionId: d.subscriptionId || "N/A",
        ...
        const formattedIps = (json.unusedIps || []).map((ip: any) => ({
            ...
            subscriptionId: ip.subscriptionId || "N/A",
  ...
  if (loading) return <div className="p-4 animate-pulse bg-gray-100 rounded-lg h-32 text-gray-500 font-medium">Ejecutando KQL global en el Tenant...</div>;

  return (
    <div className="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200">
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
        <h2 className="text-lg font-semibold text-[var(--color-primary)]">Recursos Zombi (Tenant-Wide)</h2>
          ...
          <tbody>
            {data.length > 0 ? data.map((item, i) => (
              <tr key={item.id || i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td className="p-4 text-xs font-mono text-gray-500">{item.subscriptionId.substring(0,8)}...</td>
              ...
              <tr>
                <td colSpan={5} className="p-8 text-center text-sm text-gray-500">
                  No se detectaron recursos zombie en todo el Tenant. ¡Excelente trabajo!
                </td>
              </tr>
            )}
          </tbody>
...
"""
    ...
    print("Backend y Frontend refactorizados exitosamente para soportar Tenant-Wide queries.")
