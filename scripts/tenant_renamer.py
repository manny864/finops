...
def update_api():
    path = os.path.join(base_dir, "src/app/api/tenants/route.ts")
...
def update_ui():
    path = os.path.join(base_dir, "src/app/admin/onboarding/page.tsx")
    code = """"use client";
...
def update_sop():
    path = os.path.join(base_dir, "directivas/tenant_management_SOP.md")
    with open(path, "w") as f:
        f.write("# Tenant Management SOP\\n\\n")
        f.write("- **Nomenclatura Legible**: Dado que Azure AD no siempre provee el dominio correcto mediante tokens de invitado, los SuperAdmins ahora renuevan explícitamente el nombre del tenant desde el módulo `/admin/onboarding`.\\n")
        f.write("- **API REST**: `PUT /api/tenants` actualiza `company_name` en la BD MySQL.\\n")

if __name__ == "__main__":
    update_api()
    update_ui()
    update_sop()
    print("Tenant Editor deploy completado.")
