/**
 * Genera el PDF del documento LLD con la marca CSCloudSolutions.
 *
 * Usa el mismo pipeline que generate-manual-pdfs.js (Playwright + marked),
...
const LLD_MD = path.join(process.cwd(), 'docs', 'lld', '00-lld-completo.md');
const OUTPUT_PDF = path.join(process.cwd(), 'docs', 'lld', 'LLD-FinOps-CSCloudSolutions.pdf');
...
const LOGO_TEXT_DATA_URI = 'data:image/png;base64,' +
  fs.readFileSync(path.join(process.cwd(), 'public', 'CSCloudSolutionsText.png')).toString('base64');
...
function preprocessMermaid(html) {
  // Marked genera <code class="language-mermaid"> dentro de <pre>.
  // Los reemplazamos con divs que mermaid.js procesa.
  return html.replace(
...
<div class="cover">
  ...
  <h1 class="cover-title">Low-Level Design</h1>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* --- Section dividers (horizontal rules become decorative) --- */
...
async function main() {
  if (!fs.existsSync(LLD_MD)) {
    console.error(`No se encontró: ${LLD_MD}`);
    process.exit(1);
  }

  const md = fs.readFileSync(LLD_MD, 'utf-8');
  let bodyHtml = marked.parse(md);

  // Reemplazar imágenes que apuntan a paths locales con data URIs
  bodyHtml = bodyHtml.replace(
    /src="[^"]*CSCloudSolutions\.png"/g,
    `src="${LOGO_DATA_URI}"`
  );

  // Convertir bloques mermaid a divs procesables
  bodyHtml = preprocessMermaid(bodyHtml);

  // Strip file:// links (no resolverían en el PDF) — dejar solo el texto
  bodyHtml = bodyHtml.replace(
    /<a href="file:\/\/\/[^"]*">(.*?)<\/a>/g,
    '<code>$1</code>'
  );
  ...
  console.log('Lanzando Playwright para generar PDF...');
  ...
  await page.pdf({
    ...
    headerTemplate: `
      <div style="width:100%;font-size:7px;color:#999;display:flex;justify-content:space-between;padding:0 18mm;">
        ...
        <span>Low-Level Design — FinOps Platform</span>
      </div>`,
    footerTemplate: `
  ...
  console.log(`✅ PDF generado: ${OUTPUT_PDF} (${size} KB)`);
}
