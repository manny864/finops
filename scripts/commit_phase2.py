...
def main():
    ...
    print("--- Staging de archivos Fase 2 (Seguridad y BD) ---")
    try:
        subprocess.run(["git", "add", "."], cwd=base_dir, check=True)
        commit_msg = "feat: Phase 2 - Add MySQL Docker, JWT tenant isolation, Admin Consent UI, and Key Vault"
        print(f"Ejecutando: git commit -m '{commit_msg}'")
        
        result = subprocess.run(["git", "commit", "-m", commit_msg], cwd=base_dir)
        
        if result.returncode == 0:
            print("\nCommit generado exitosamente en tu rama principal.")
        else:
